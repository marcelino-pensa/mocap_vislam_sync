...
class HectorVislamSync {
 public:
    HectorVislamSync(ros::NodeHandle *nh);

 private:
    ros::NodeHandle nh_;
    std::string in_lidar_topic_, in_vislam_topic_;
    std::string output_file_path_;
    boost::shared_ptr<Sync> sync_;
    message_filters::Subscriber<geometry_msgs::PoseStamped>* lidar_sub_;
    ...
    Eigen::Vector3d first_pos_lidar_, first_pos_vislam_;
    ros::ServiceServer start_scale_estimation_srv_, stop_scale_estimation_srv_;
    std::string node_name_;

    // Callback that subscribes to both lidar and vislam measurements
    void PoseCallback(const geometry_msgs::PoseStamped::ConstPtr& lidar_msg,
