...
void HectorVislamSync::PoseCallback(const geometry_msgs::PoseStamped::ConstPtr& lidar_msg,
    ...
    if (start_new_estimation_) {
        ...
        initial_time_ = lidar_msg->header.stamp;
    }
    ...
    times_.push_back((lidar_msg->header.stamp - initial_time_).toSec());
...
bool HectorVislamSync::StopSyncSrv(std_srvs::Trigger::Request  &req,
    ...
    for (uint i = 0; i < position_pair_vec_.size(); i++) {
       ...
       x_file << lidar_pos[0] << ", " << vislam_pos[0] << ", " << times_[i] << std::endl;
       y_file << lidar_pos[1] << ", " << vislam_pos[1] << ", " << times_[i] << std::endl;
       z_file << lidar_pos[2] << ", " << vislam_pos[2] << ", " << times_[i] << std::endl;
    }
    x_file.close();
    y_file.close();
    z_file.close();
